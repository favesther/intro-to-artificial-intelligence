...
def read_data():
  ...
  print(data_as_strings)
  TRAINING_DATA = [[map(float(f1), float(f2))[0],map(float(f1), float(f2))[1], int(c)] for [f1, f2, c] in data_as_strings]
  # # remap training data
  # for data in TRAINING_DATA:
  #     change = map(data[0],data[1])
  #     data[0] = change[0]
  #     data[1] = change[1]
...
for i in range(N_EPOCHS):
  ...
  if changed_count==0: 
    print("Converged in ",i," epochs."); break
